...
- Title-index ambiguity resolution: a title text can map to multiple AIDs (e.g. main series + an OVA/movie sharing the same title). `FindCandidates` first narrows to the best-ranked title type (`main` > `official` > other) — a `main` hit wins outright over a `synonym` collision. If more than one AID survives at that rank, `SearchAnime` fetches each candidate (respecting cache + 2s pacing) and filters by release year; 0 or >1 year matches still error as ambiguous, surfacing the `--q aid:<id>` hint.
- Episode detection for TV accepts `SxxEyy`, `S01.E01`, `S01 E01`, and `NxYY`; for anime it additionally handles bare-dash, hash, OVA/SP, and Part (arabic + roman) markers. The `SxxEyy`/`NxYY` episode digit groups allow up to 3 digits (`\d{1,3}`) — a 2-digit cap previously truncated `S01E100`/`E101`/`E102` to `E10`, colliding three episodes onto one target file. `extractEpisode` (the `SxxEyy`/`NxYY` matcher) runs before the anime-specific fallback patterns and short-circuits on any match, so this cap had to be fixed at the shared pattern, not just in the anime fallbacks (which already used `\d{1,3}`).
